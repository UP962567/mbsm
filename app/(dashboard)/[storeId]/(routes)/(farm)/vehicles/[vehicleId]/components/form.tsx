...
import { FarmEquipment, FarmLocation, FarmVehicle } from "@prisma/client"
...
export const Former: React.FC<FormProps> = ({
  ...
  const onSubmit = async () => {
    ...
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/vehicles/${params.vehicleId}`, data);
      } else {
        await axios.post(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/vehicles`, data);
      }
      router.refresh();
      router.push(`/${params.storeId}/vehicles`);
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/vehicles/${params.vehicleId}`);
      router.refresh();
      router.push(`/${params.storeId}/vehicles`);
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      disabled={loading}
                      placeholder="Information of the gass type"
                      value={field.value || ''}
                      onChange={(event) => {
                        field.onChange(event);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="Total numbers of vehicles"
                      type="number"
                      value={field.value || ''}
                      onChange={(event) => {
