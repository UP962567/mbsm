...
import { FarmMedicine } from "@prisma/client"
...
export const Former: React.FC<FormProps> = ({
  ...
  const title = initialData ? 'Edit Medicine' : 'Create Medicine';
  const description = initialData ? 'Edit a Medicine.' : 'Add a new Medicine';
  const toastMessage = initialData ? 'Medicine updated.' : 'Medicine created.';
  ...
  const onSubmit = async () => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/medicines/${params.medicineId}`, data);
      } else {
        await axios.post(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/medicines`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/medicines/${params.medicineId}`);
      router.refresh();
      router.push(`/${params.storeId}/medicines`);
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      disabled={loading}
                      placeholder="Information about the medicine"
                      value={field.value || ''}
                      onChange={(event) => {
                        field.onChange(event);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="Total numbers of medicines bought"
                      type="number"
                      value={field.value || ''}
                      onChange={(event) => {
                        field.onChange(event);
