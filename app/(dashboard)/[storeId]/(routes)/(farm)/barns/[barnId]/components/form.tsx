...
import { FarmField, FarmLocation, FarmBarn } from "@prisma/client"
...
export const Former: React.FC<FormProps> = ({
  ...
  const title = initialData ? 'Edit Barn' : 'Create Barn';
  const description = initialData ? 'Edit a Barn.' : 'Add a new Barn';
  const toastMessage = initialData ? 'Barn updated.' : 'Barn created.';
  ...
  const onSubmit = async () => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/barns/${params.barnId}`, data);
      } else {
        await axios.post(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/barns`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/${process.env.NEXT_PUBLIC_API_URL}/${params.storeId}/barns/${params.barnId}`);
      router.refresh();
      router.push(`/${params.storeId}/barns`);
