...
export async function GET(
  ...
  try {
    ...
    const product = await prismadb.farmWorker.findUnique({
      where: {
...
export async function DELETE(
  req: Request,
  { params }: { params: { workerId: string, storeId: string } }
) {
  try {
    const { userId } = auth();
    ...
    const product = await prismadb.farmWorker.deleteMany({
      where: {
        uuid: params.workerId,
      }
    });
...
export async function PATCH(
  ...
  try {
    ...
    const product = await prismadb.farmWorker.update({
      where: {
        uuid: params.workerId,
      },
      data: {
