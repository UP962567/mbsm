...
export async function GET(
  ...
  try {
    ...
    const product = await prismadb.farmFeed.findUnique({
      where: {
        uuid: params.feedId
      }
    });
...
export async function DELETE(
  req: Request,
  { params }: { params: { feedId: string, storeId: string } }
) {
  try {
    const { userId } = auth();
    ...
    const product = await prismadb.farmFeed.deleteMany({
      where: {
        uuid: params.feedId,
      }
    });
...
export async function PATCH(
  ...
  try {
    ...
    const product = await prismadb.farmFeed.update({
      where: {
        uuid: params.feedId,
      },
      data: {
